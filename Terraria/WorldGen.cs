	...
	internal class WorldGen
	{
							...
							using (BinaryWriter binaryWriter = new BinaryWriter(fileStream))
							{
								...
								for (int i = 0; i < Main.maxTilesX; i++)
								{
									...
									for (int j = 0; j < Main.maxTilesY; j++)
									{
                                        ...
                                        if (Main.tile[i, j].type == 254)
                                        {
                                            binaryWriter.Write((Int32)Main.tile[i, j].portalPartner.X);
                                            binaryWriter.Write((Int32)Main.tile[i, j].portalPartner.Y);
                                        }
						...
						else
						{
							...
							for (int j = 0; j < Main.maxTilesX; j++)
							{
								...
								for (int k = 0; k < Main.maxTilesY; k++)
								{
                                    ...
                                    //Mod: Portal functionality
                                    if (Main.tile[j, k].type == 254)
                                    {
                                        Main.tile[j, k].portal = true;
                                        Main.tile[j, k].portalPartner.X = binaryReader.ReadInt32();
                                        Main.tile[j, k].portalPartner.Y = binaryReader.ReadInt32();
                                    }
		...
		public static void Check2xX(int i, int j, byte type)
		{
			...
			for (int l = 0; l < num3; l++)
			{
				...
				{
					Main.tile[num + 1, num2 + l] = new Tile();
				}
																					...
																					if (type == 105)
																					{
																						...
																						else
																						{
																							...
																							else
																							{
																								if (num26 == 8)
																								{
																									if (WorldGen.checkMech(i, j, 30) && NPC.MechSpawn((float)num27, (float)num28, 55))
																									{
																								...
																								else
																								{
																									if (num26 == 9)
																									{
																										if (WorldGen.checkMech(i, j, 30) && NPC.MechSpawn((float)num27, (float)num28, 46))
																										{
